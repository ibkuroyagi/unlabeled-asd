...
def _module_test(n_fft, device="cuda:0"):
    ...
    # y = FFTEncoderLayer(18, 16000)(FFT()(x))
    # print(y.shape)
    # y = TimeEncoderLayer(18, 16000)(x)
    # loss = torch.mean(y**2)
    # loss.backward()
    # print(y.shape)
    # breakpoint()
    ...
    # stft = STFT(**stft_cfg)
    # x_stft = stft(x).to(device)
    # spectrogram_size = x_stft.shape[1:]
    # print(spectrogram_size)
    # stft_encoder = STFT2dEncoderLayer(spectrogram_size)
    # # if min(spectrogram_size) >= 70:
    # #     stft_encoder = STFT2dEncoderLayer(spectrogram_size)
    # # elif spectrogram_size[0] >= 70 and spectrogram_size[1] < 70:
    # #     stft_encoder = STFT1dEncoderLayer(spectrogram_size, 1)
    # # elif spectrogram_size[0] < 70 and spectrogram_size[1] >= 70:
    # #     stft_encoder = STFT1dEncoderLayer(spectrogram_size, 2)
    # # else:
    # #     raise ValueError("input sequence is too short")
    # stft_encoder.to(device)
    # y = stft_encoder(x_stft)
    print("----------------------------------------------")
    # loss = torch.mean(y**2)
    # loss.backward()
